...
N_intro = floor(Fs*0.4);
x_intro = randn(1,N_intro);	%noise sequence
x_intro_int16 = int16(x_intro*2^12);

wavplayex( [x_intro_int16; x_intro_int16], Fs); %start playing thread
...
for ( f_index = [1:N_f_delay] )   
   ...
   level = wavplayex([x;x],Fs); 
   %fprintf(1,'%3.0f\n',level);
   if ( level==0 ) warning('wavplay buffer underun'); end
end

%estimate play-record delay with left channel only
N_intro_window = 200;
...
x_ccf = filter( fliplr(x_intro_window(1,:)), 1, x_intro ); %cross correlation
[a,b] = max(abs(x_ccf));
...
for ( f_index = [1:length(f)] )
   ...
   H_left(f_index) = x_complex(1:N)*transpose(y(1,1:N))/N;   
   H_right(f_index) = x_complex(1:N)*transpose(y(2,1:N))/N; 
...
subplot(2,2,1);
if (frequency_scale_log == 1)
   semilogx(f/1000,20*log10(abs(H_left) + EPSILON ),'.-');
else
   plot(f/1000,20*log10(abs(H_left) + EPSILON ),'.-');
end
axis([0,f_max/1000,-50,0]);
grid on;
title('stereo left/right out, stereo left in');
xlabel('frequency (kHz)');
ylabel('gain (dB)');

subplot(2,2,2);
if (frequency_scale_log == 1)
   semilogx(f/1000,20*log10(abs(H_right) + EPSILON ),'.-');
else
   plot(f/1000,20*log10(abs(H_right) + EPSILON ),'.-');
end
axis([0,f_max/1000,-50,0]);
grid on;
title('stereo left/right out, stereo right in');
xlabel('frequency (kHz)');
ylabel('gain (dB)');


phase_rotator = exp( -j*angle(H_left(1)) );
subplot(2,2,3);
if (frequency_scale_log == 1)
   semilogx(f/1000,angle(H_left*phase_rotator)/pi,'.-');
else
   plot(f/1000,angle(H_left*phase_rotator)/pi,'.-');
end
...
subplot(2,2,4);
if (frequency_scale_log == 1)
   semilogx(f/1000,angle(H_right*phase_rotator)/pi,'.-');
else
   plot(f/1000,angle(H_right*phase_rotator)/pi,'.-');
end
axis([0,f_max/1000,-1.1,1.1]);
grid on;
title('stereo left/right out, stereo right in');
