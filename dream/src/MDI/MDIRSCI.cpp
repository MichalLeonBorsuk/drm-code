...
bool
	CDownstreamDI::AddSubscriber(const string& dest, const char profile, const int iSubsamplingFactor)
{
...
bool CDownstreamDI::SetOrigin(const string& origin)
{
    if (origin != "")
    {
	int port = atoi(origin.c_str());
	// TODO set listening port
    }
    return false;
}
...
void CDownstreamDI::StartRSIRecording(CParameter& Parameter, char cPro)
{
...
void CDownstreamDI::StopRSIRecording()
{
