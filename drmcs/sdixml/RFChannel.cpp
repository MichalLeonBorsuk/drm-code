...
{
    if(n==NULL) {
        cerr << "RFChannel misconfiguration of msc protection level - no config data" << endl;
...
{
    if(misconfiguration) {
        cerr << "RFChannel GetParams already misconfigured" << endl;
    }
...
void RFChannel::ReConfigure(xmlNodePtr config)
{
    ...
    if(misconfiguration) {
        cerr << "RFChannel misconfiguration detected by superclass!" << endl;
        return;
    }
