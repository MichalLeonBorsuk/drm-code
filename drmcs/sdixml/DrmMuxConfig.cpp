...
void DrmMuxConfig::ReConfigure(xmlNodePtr config)
{
    ...
    if(misconfiguration) {
        cerr << "DrmMuxConfig misconfiguration detected by superclass!" << endl;
        return;
    }
...
void DrmMuxConfig::GetParams(xmlNodePtr n)
{
    ...
    if(!xmlStrcmp(n->name, BAD_CAST "channel")) {
        ...
        if(channel.misconfiguration) {
            misconfiguration = true;
            cerr << "channel misconfigured" << endl;
        }
    }
    if(xmlStrEqual(n->name, BAD_CAST "streams")) {
        for(xmlNodePtr c=n->children; c; c=c->next) {
            if(c->type==XML_ELEMENT_NODE) {
                ...
                s.ReConfigure(c);
                stream.push_back(s);
                if(s.misconfiguration) {
                    misconfiguration = true;
                    cerr << "stream misconfigured" << endl;
                }
            }
        }
    }
    if(!xmlStrcmp(n->name, BAD_CAST "services")) {
        for(xmlNodePtr c=n->children; c; c=c->next) {
            if(c->type==XML_ELEMENT_NODE) {
                ...
                if(s.misconfiguration) {
                    misconfiguration = true;
                    cerr << "service misconfigured" << endl;
                }
            }
        }
    }
    if(!xmlStrcmp(n->name, BAD_CAST "afs")) {
        afs.ReConfigure(n);
        if(afs.misconfiguration) {
            misconfiguration = true;
            cerr << "afs misconfigured" << endl;
        }
    }
}
