...
{
    m_Polynomial = 0x1021;
    m_Reflect = false;	// use true to reverse bits of input bytes
    m_ReflectCrc = false;	// use true to reverse bits of output crc
    m_CrcXorOutput = 0xffff;	// use 0xFFFF to invert crc output
    m_ResetValue = 0xffff;	// Most commonly 0 or 0xFFFF
    m_Crc = m_ResetValue;
    if(no_table) {
        CreateTable();
        no_table=false;
